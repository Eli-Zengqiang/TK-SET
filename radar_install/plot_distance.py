...
def computer_dis(x, y, z, i):
    Tx = [-0.5967, 0.6141, -0.0191]
    Ty = [0.6514, 0.0490, 0.8550]
    Tz = [-0.0907, 0.0088, 0.0157]
...
def computer_signal_npz_std(i, npz_path=r"E:\2.1SE-T32\计算误差距离\npy文件\1号楼\1号楼radar0.0_20.2.npy"):
    '''
    计算单个npz文件不同距离点的误差
    :return:
    '''
...
def computer_all_dir(index, current_dir=r"E:\2.1SE-T32\计算误差距离\npy文件\0号楼"):
    # E:\2.1SE-T32\计算误差距离\npy文件\0号楼
    # E:\2.1SE-T32\计算误差距离\npy文件\1号楼
    # E:\2.1SE-T32\计算误差距离\npy文件\2楼平台
...
def draw_sca(labels_lists,A):  # label_lists,A:距离
    ...
    for i, (row, col) in enumerate(subplot_positions, 1):  #i作为位置0,1,2号楼
        ...
        if i ==4:
           ...
           ####################### 设置色彩##########################
            ...
            scatter1 = ax.scatter(X[i - 1][A], Y[i - 1][A], Z[i - 1][A], c=colors,label='2mm一个颜色分级,大于2cm灰色', marker='o')
            '''
    ...
    for i, color2 in enumerate(colors1):  #A:4个距离
        ax1.scatter(X[i][A], Y[i][A], Z[i][A], c=color2, alpha=0.7)
    ...
    # #
    # # # 添加标签到 colorbar
    # # cbar.set_label('色阶图', rotation=270, labelpad=15)
