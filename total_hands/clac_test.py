...
class ClacTestThread(QThread):
    ...
    def run_exe(self,txt_path,dir_path):
        try:
            ...
            points_PC = np.loadtxt(txt_path)[:, 0:3]   #points_PC合成点云
            ...
            a=[]
            out_strs=[]
            points_TS_all = np.empty([0, 4])  #创建一个空数组,相对于一个一维空列表：[]
            for path in os.listdir(dir_path):
                if path[-4:] == ".txt":
                    t_path = os.path.join(dir_path, path)
                    points_TS = np.loadtxt(t_path)[:, 0:3]    #points_TS全站仪打点点云
                    ...
                    for i in range(len(points_TS)):
                        ...
                        try:
                            plane = sko.Plane.best_fit(neibr_points)#用合成点云形成面
                            points_TS[i, 3] = plane.distance_point_signed(points_TS[i, 0:3])#计算点（全站仪打点）到平面距离（误差），并将误差放到点的最后一列
