...
class Inclinometer:
    ...
    def __init__(self,com_name="/dev/ttyS4",bound_raito=9600):#__init__(self,com_name="/dev/ttyS4",bound_raito=9600):
    ...
    def _get_inc(self):
        get_state=False
        self.com.write([0x68,0x04,0x00,0x04,0x08])
        time.sleep(0.1)
    ...
    def _get_abs_zero(self):
        print("_get_abs_zero")
        self.com.write([0x68,0x04,0x00,0x0D,0x11])
        time.sleep(1)
        start_time=time.time()
        while True:
            if self.com.in_waiting:
...
if __name__ == "__main__":
    outputfile=""
    try:
        opts, args = getopt.getopt(sys.argv[1:], "ho:z:a:", ["ofile="])
