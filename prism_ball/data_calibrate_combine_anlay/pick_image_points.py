...
def extract_points_2D(path):
    ...
    save_path = os.path.join(os.path.dirname(path), "标靶2D.txt")
    if (len(corners) >= 6):
        with open(save_path, "w") as f:
            for corner in corners:
                f.write(f"{int(corner[0])} {int(corner[1])}\n")
        print(f"已保存 {len(corners)} 个点 → {save_path}")
    else:
        print(f'点数不足（当前 {len(corners)} 个），需要至少 6 个点才保存')
...
if __name__ == '__main__':
    if len(sys.argv) == 2:
        ...
        input_path = r"G:\TK-set\2.1data\20260807\b855\0\1.jpg"
    extract_points_2D(input_path)
