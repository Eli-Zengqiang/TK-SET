...
'''
# def remove_logs_existd(folder=None):
#     """删除目标文件夹中的所有文件（保留文件夹本身）；路径不存在时静默跳过"""
#     if folder is None or not folder or not os.path.isdir(folder):
#         return
#     for item in os.listdir(folder):
#         item_path = os.path.join(folder, item)
#         if os.path.isfile(item_path):
#             os.remove(item_path)
# def files_combined_dir():
#     """返回 files_combined 目录：冻结时与运行 exe 同目录，开发态为当前脚本目录"""
#     if getattr(sys, "frozen", False):
#         return os.path.join(os.path.dirname(os.path.abspath(sys.executable)), "files_combined")
#     return os.path.join(os.path.dirname(os.path.abspath(__file__)), "files_combined")
# def remove_logs_combined():
#     """将程序目录下的非 .py 文件移动到 files_combined（保存运行时产生的其它文件）；目录不存在时静默跳过"""
#     dst_dir = files_combined_dir()
#     src_dir = os.path.dirname(dst_dir)
...
def combine_Totalstation_points(datas_folder,total_signal,signal_combine,hand_angles=None):  #combine_Totalstation_points(datas_folder,total_points_file,total_signal):
    ...
    if total_signal == 2:  #此处是读取全站仪打点文件
    ...
    for dir_id,dir in enumerate(datas_folders):
        ...
        if total_signal == 2:
            ...
            print(f"修改cfg路径：{original_CFG_path}")
            ...
            if hand_angles:
                sa.TotalStationOffsetAngle_cfg(hand_angles[3])   #写入全站仪标定角度Z
                sa.corret_file_cfg(hand_angles[0],hand_angles[2]) #写入雷达标定的X、Z
            if sig == 1:
...
if __name__=="__main__":
    ...
    datas_folder=r"G:\TK-set\2.1data\20260807\b855/"   #输入批处理数据文件夹，如果需要后方交会，检查是否有全站仪.txt
